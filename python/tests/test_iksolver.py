...
def test_iksolver():
    ...
    for s in setting_list:
        av_sol = fksolver.solve_inverse_kinematics(
                s.pose_desired, s.av_init, end_link_id, rarm_joint_ids, with_base=s.with_base, option=option)
        with_rot = (len(s.pose_desired)==6)
        P, J = fksolver.solve_forward_kinematics([av_sol], [end_link_id], rarm_joint_ids, with_rot=with_rot, with_base=s.with_base)
        pose = P[0]
...
def test_multiple_iksolver():
    ...
    pose_desired_list = [np.array([0.7, -0.6, 0.8, 0, 0, 0]), np.array([0.7, +0.6, 0.8])]

    av_sol = fksolver.solve_multi_endeffector_inverse_kinematics(
            pose_desired_list, angle_vector, end_link_ids, joint_ids, with_base=True, option=option)
    pose_list, _ = fksolver.solve_forward_kinematics([av_sol], end_link_ids, joint_ids, with_rot=True, with_base=True)
